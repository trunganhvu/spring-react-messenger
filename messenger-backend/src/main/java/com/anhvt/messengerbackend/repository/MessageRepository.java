...
public interface MessageRepository extends JpaRepository<MessageEntity, Integer> {
    ...
    @Query(value = "SELECT * FROM (SELECT * FROM message m " +
            "WHERE " +
            "   m.msg_group_id=:id " +
            "   AND ((:offset > 0 AND m.id < :offset) OR (:offset <= 0)) " +
            "ORDER BY m.id DESC LIMIT 20)t order by id", nativeQuery = true)
    List<MessageEntity> findByGroupIdAndOffset(@Param(value = "id") int id, @Param(value = "offset") int offset);

    @Query(value = "" +
            "SELECT * FROM (SELECT * FROM message m WHERE m.msg_group_id=:id ORDER BY m.id DESC LIMIT 20)t " +
